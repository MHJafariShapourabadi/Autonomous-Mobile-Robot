...
class DepthCameraFrameIDConverterNode : public rclcpp::Node
{
    private:
    ...
    public:
        DepthCameraFrameIDConverterNode(): Node("depth_camera_frame_id_converter")
        {
            this->declare_parameter("frame_id", "depth_camera_link");
            this->declare_parameter("subscription_topic", "/depth_camera/points");
            this->declare_parameter("publisher_topic", "/depth_camera/points/corrected");
...
void DepthCameraFrameIDConverterNode::subscriber_callback(const PointCloud2::SharedPtr msg)
{
    auto new_msg = *msg;
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<DepthCameraFrameIDConverterNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
