...
class NoisyOdom(Node):
    def __init__(self):
        ...
        self._wheel_radius_error_ = self.get_parameter('wheel_radius_error').get_parameter_value().double_value
        self._wheel_separation_error_ = self.get_parameter('wheel_sepaparion_error').get_parameter_value().double_value
        self._noise_std_ = self.get_parameter('noise_std').get_parameter_value().double_value

        self._wheel_radius_ += self._wheel_radius_error_
        self._wheel_separation_ += self._wheel_separation_error_
        ...
        self._odom_msg_.child_frame_id = "base_footprint"
        ...
        self._odom_base_tf_.child_frame_id = "base_footprint"

    def _joint_states_callback(self, msg: JointState):
        current_time = Time.from_msg(msg.header.stamp)
        current_wheel_pos_left = msg.position[0] + np.random.normal(0, self._noise_std_)
        current_wheel_pos_right = msg.position[1] + np.random.normal(0, self._noise_std_)
