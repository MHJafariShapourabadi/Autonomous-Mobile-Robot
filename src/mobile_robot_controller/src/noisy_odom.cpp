#include "mobile_robot_controller/noisy_odom.hpp"

NoisyOdom::NoisyOdom() :
    Node("noisy_odom"), wheel_radius(0.0), wheel_separation(0.0), 
    wheel_radius_error(0.0), wheel_separation_error(0.0), noise_std(0.0), seed(0), 
    last_wheel_pos(0, 0), robot_pos(0, 0), robot_orein(0), robot_speed(0, 0), last_time(now())
{
    ...
    declare_parameter("wheel_radius_error", 0.005);
    declare_parameter("wheel_separation_error", 0.02);
    declare_parameter("noise_std", 0.005);

    wheel_radius = get_parameter("wheel_radius").as_double();
    wheel_separation = get_parameter("wheel_separation").as_double();
    wheel_radius_error = get_parameter("wheel_radius_error").as_double();
    wheel_separation_error = get_parameter("wheel_separation_error").as_double();
    noise_std = get_parameter("noise_std").as_double();

    seed = std::chrono::system_clock::now().time_since_epoch().count();
    rng = std::default_random_engine(seed);
    normal_dist = std::normal_distribution(0.0, noise_std);
    ...
    odom_base_tf.child_frame_id = "base_footprint";

}

void NoisyOdom::joint_states_callback(const JointState& msg)
{
    auto current_time = rclcpp::Time(msg.header.stamp);
    double current_wheel_pos_left = msg.position.at(0) + normal_dist(rng);
    double current_wheel_pos_right = msg.position.at(1) + normal_dist(rng);
...
int main(int argc, char ** argv)
{
    ...
    auto node = std::make_shared<NoisyOdom>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}